/**
 * Session timeout: 20 minutes. When ≤2 min remain, show modal "Stay logged in?".
 * Stay → extend by 2 min. Timeout or Logout → POST /api/auth/logout, clear storage, redirect to /admin/login.
 */
...
type SessionTimeoutContextValue = {
  /** Call when user does an action that should extend session (only extends when in warning state). */
  extendSession: () => void;
};

const SessionTimeoutContext = createContext<SessionTimeoutContextValue | null>(null);

export function useSessionTimeout(): SessionTimeoutContextValue | null {
...
export function SessionTimeoutProvider({ children }: { children: ReactNode }) {
  ...
  const value: SessionTimeoutContextValue = { extendSession };
        ...
        <View style={styles.overlay}>
          <View style={styles.card}>
            ...
            <View style={styles.actions}>
              ...
              <Pressable style={styles.btnPrimary} onPress={handleStay}>
...
const styles = StyleSheet.create({
  ...
  btnPrimary: { backgroundColor: '#4F46E5', paddingVertical: spacing[2], paddingHorizontal: spacing[4], borderRadius: 8 },
