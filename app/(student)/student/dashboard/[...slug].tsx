/**
 * Student dashboard module placeholder: any path under /student/dashboard/*.
 */

import { useRouter, useLocalSearchParams } from 'expo-router';
import { View, Text, StyleSheet, Pressable } from 'react-native';
import { useStudentOrNull } from '@/lib/student-context';
import { spacing } from '@/theme/spacing';
import { textStyles } from '@/theme/typography';

const STUDENT_BLUE = '#2563EB';

export default function StudentModulePlaceholderScreen() {
  const router = useRouter();
  const params = useLocalSearchParams<{ slug?: string[] }>();
  const slug = params.slug ?? [];
  const pathLabel = slug.map((s) => s.replace(/-/g, ' ')).join(' / ') || 'Module';
  const student = useStudentOrNull();
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.content}>
        ...
        {student?.school_code ? (
          <Text style={styles.hint}>School: {student.school_code}</Text>
        ) : null}
