/**
 * Password Manager (manage_passwords only): list credentials, reset staff/student password.
 * Same APIs as admin: GET login-credentials, POST staff/students reset-password.
 */
...
type Credential = { id?: string; type: 'staff' | 'student'; staff_id?: string; admission_no?: string; student_id?: string; name?: string; [k: string]: unknown };

export default function TeacherPasswordScreen() {
  const router = useRouter();
  const { schoolCode, hasPermission } = useTeacher();
  ...
  const { data: credsRes, isLoading, isError, error, refetch } = useQuery({
    ...
    enabled: Boolean(schoolCode) && hasPermission('manage_passwords'),
  });
  ...
  const credentials = [...staffList.map((s) => ({ ...s, type: 'staff' as const })), ...studentList.map((s) => ({ ...s, type: 'student' as const }))];

  const resetStaffMutation = useMutation({
    mutationFn: (body: { school_code: string; staff_id: string; new_password: string }) => passwordService.resetStaffPassword(body),
    onSuccess: () => {
    ...
    onError: (err: Error & { response?: { data?: { message?: string } } }) => showToast(err?.response?.data?.message ?? err?.message ?? 'Reset failed', 'error'),
  });
  const resetStudentMutation = useMutation({
    mutationFn: (body: { school_code: string; admission_no?: string; student_id?: string; new_password: string }) => passwordService.resetStudentPassword(body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['login-credentials', schoolCode] });
      showToast('Password reset', 'success');
      setResetModal(null);
  ...
  const handleReset = useCallback(() => {
    ...
    if (resetModal.type === 'staff') {
      ...
      if (!staffId) { showToast('Staff ID missing', 'error'); return; }
      ...
      if (!admissionNo) { showToast('Student ID missing', 'error'); return; }
      resetStudentMutation.mutate({ school_code: schoolCode, admission_no: String(admissionNo), new_password: newPassword });
    }
    ...
    return (
      <View style={styles.root}>
        ...
        <View style={styles.centered}>
          <Text style={styles.noPerm}>You do not have permission to manage passwords.</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={styles.root}>
      ...
      {isLoading && !credentials.length ? (
        <View style={styles.centered}><ActivityIndicator size="large" color={G} /></View>
      ) : isError ? (
        <View style={styles.centered}>
          <Text style={styles.errorText}>{(error as Error)?.message ?? 'Failed to load'}</Text>
          <Pressable style={styles.retryBtn} onPress={() => refetch()}><Text style={[styles.retryText, { color: G }]}>Retry</Text></Pressable>
        </View>
      ) : credentials.length === 0 ? (
        <View style={styles.centered}><EmptyState title="No credentials" message="No staff or student login credentials found." /></View>
      ) : (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          ...
          {credentials.map((c, i) => {
            ...
            return (
              <Pressable key={id} style={styles.card} onPress={() => { setResetModal(c); setNewPassword(''); setConfirmPassword(''); }}>
                <View style={styles.cardRow}>
                  <Text style={styles.cardLabel}>{label}</Text>
                  <Text style={styles.cardName}>{c.name ?? '—'}</Text>
                </View>
                <Text style={[styles.resetHint, { color: G }]}>Tap to reset password</Text>
              </Pressable>
            );
          })}
        </ScrollView>
        ...
        <Pressable style={styles.modalOverlay} onPress={() => setResetModal(null)}>
          <Pressable style={styles.modalCard} onPress={(e) => e.stopPropagation()}>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
              ...
              <FloatingLabelInput label="Confirm password" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry />
              <View style={styles.modalActions}>
                <Pressable style={styles.btnOutline} onPress={() => setResetModal(null)}><Text style={styles.btnOutlineText}>Cancel</Text></Pressable>
                <Pressable style={[styles.btnPrimary, { backgroundColor: G }]} onPress={handleReset} disabled={resetStaffMutation.isPending || resetStudentMutation.isPending}>
...
const styles = StyleSheet.create({
  ...
  noPerm: { ...textStyles.body, color: '#6B7280', textAlign: 'center' },
  ...
  resetHint: { ...textStyles.caption, marginTop: spacing[2] },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', padding: spacing[6] },
  modalCard: { backgroundColor: '#fff', borderRadius: 12, padding: spacing[6] },
  modalActions: { flexDirection: 'row', gap: spacing[4], marginTop: spacing[6], justifyContent: 'flex-end' },
  btnOutline: { paddingVertical: spacing[2], paddingHorizontal: spacing[4], borderWidth: 1, borderColor: G, borderRadius: 8 },
  btnOutlineText: { ...textStyles.button, color: G },
