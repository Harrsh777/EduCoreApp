/**
 * Digital Diary / Homework: GET /api/diary, POST /api/diary. List and upload.
 */
...
export default function HomeworkScreen() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['diary', schoolCode],
    queryFn: () => diaryService.getDiary(schoolCode).then((r) => r.data),
    enabled: Boolean(schoolCode),
  });

  const entries = (data as { data?: unknown[] })?.data ?? (Array.isArray(data) ? data : []);
  ...
  return (
    <View style={styles.root}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        {isLoading && !Array.isArray(entries)?.length ? (
          <ActivityIndicator size="small" color={INDIGO} style={styles.loader} />
        ) : Array.isArray(entries) && entries.length > 0 ? (
          (entries as { id?: string; title?: string }[]).map((e, i) => (
            <View key={e.id ?? i} style={styles.card}>
              <Text style={styles.cardTitle}>{e.title ?? '—'}</Text>
            </View>
          ))
        ) : (
          <Text style={styles.empty}>No diary entries. Upload homework in the web app.</Text>
        )}
      </ScrollView>
    </View>
  );
